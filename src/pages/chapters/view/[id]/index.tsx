...
import { useChapterFindFirst, useRoqClient } from 'lib/roq';

import { ChapterInterface } from 'interfaces/chapter';

function ChapterViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const roqClient = useRoqClient();
  const queryParams = useMemo<{ include: { book: boolean } }>(
    () =>
  ...
  const { data, error, isLoading } = useChapterFindFirst(queryParams, {}, { disabled: !id });
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('chapter', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/chapters/edit/${id}`} passHref legacyBehavior>
                    <Button
