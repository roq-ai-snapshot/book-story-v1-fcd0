...
import { useRoqClient, useChapterFindFirst } from 'lib/roq';
...
function ChapterEditPage() {
  ...
  const { data, error, isLoading, mutate } = useChapterFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: ChapterInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.chapter.update({
        data: values as RoqTypes.chapter,
        where: {
          id,
        },
      });
